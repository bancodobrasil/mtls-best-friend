...
export default function ItemTestWithInvalidCertificate({ url, method }) {
  ...
  if (error && error.info?.match(/error/g)) {
    return <StatusedItem message="Expected response (invalid cert): <code>400 Bad Request</code>" status="OK" prefix={method} />;
  }
  if (error) {
    return <StatusedItem message="Expected response (invalid cert): <code>400 Bad Request</code>" status="FAIL" prefix={method} />;
  }
  if (data) {
    return <StatusedItem message="Expected response (invalid cert): <code>400 Bad Request</code>" status="FAIL" prefix={method} />;
  }
  return <StatusedItem message="Expected response (invalid cert): <code>400 Bad Request</code>" status="LOADING" prefix={method} />;
}
