import useSWR from "swr";
import { fetcher } from "../../components/fetcher";
import StatusedItem from "./statusedItem";

export default function ItemTestWithInvalidCertificate({ url }) {
  const key = `-----BEGIN EC PRIVATE KEY-----\nMHcCAQEEINdmPSTTw6c1zRFfGsuZigb2aUXE4cq9RCs5fjVKHQcGoAoGCCqGSM49\nAwEHoUQDQgAE49ktXBKcGcZ6phlac6SWZmkTwFwWkEfYw3vuiEc5igiIrzcoQeOY\ngb/tNKD7QuSJcm4i5WyF/KtfFyah+TOyoQ==\n-----END EC PRIVATE KEY-----`;
  const certificate = `-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----`;
  const ca = `-----BEGIN CERTIFICATE-----\\nvZ0jeKOCMu17VPwJostVWp07NkApTYMTsikC3h1l\n-----END CERTIFICATE-----`;

  const { data, error } = useSWR(["/api/request-mtls-server", JSON.stringify({ url, key, certificate, ca }), "POST"], fetcher);

  if (error && error.info.match(/error/g)) {
    return <StatusedItem i18nMessaKey="server.sucess400InvalidCert" status="OK" />;
  }
  if (data) {
    return <StatusedItem i18nMessaKey="server.sucess400InvalidCert" status="FAIL" />;
  }
  return <StatusedItem i18nMessaKey="server.sucess400InvalidCert" status="LOADING" />;
}
