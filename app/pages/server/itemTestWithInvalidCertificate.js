...
export default function ItemTestWithInvalidCertificate({ url, method }) {
  ...
  const { data, error } = useSWR(["/api/request-mtls-server", JSON.stringify({ url, key, certificate, ca }), method], fetcher);

  if (error && error.info?.match(/error/g)) {
    return <StatusedItem i18nMessaKey="server.sucess400InvalidCert" status="OK" prefix={method} />;
  }
  if (error) {
    return <StatusedItem i18nMessaKey="server.sucess400InvalidCert" status="FAIL" prefix={method} />;
  }
  if (data) {
    return <StatusedItem i18nMessaKey="server.sucess400InvalidCert" status="FAIL" prefix={method} />;
  }
  return <StatusedItem i18nMessaKey="server.sucess400InvalidCert" status="LOADING" prefix={method} />;
}
